...
const rarityOrder = ["Exotic", "Remarkable", "High Grade", "Base Grade"];

export default function Tournament() {
    const [stickers, setStickers] = useState<graffiti[]>([]);
    ...
    const fetchStickers = async () => {
        try {
            const response = await axios.get(
                "https://api.cs2data.info/en/graffiti.json",
            );
            const filteredStickers: graffiti[] = response.data.filter(
    ...
    const orderStickersByRarity = (stickers: graffiti[]) => {
