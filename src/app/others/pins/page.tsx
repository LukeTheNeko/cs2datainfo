...
export default function PinsAll() {
  ...
  const fetchStickersAndCrates = async () => {
    try {
      // Substituindo Axios por Fetch para pegar os dados dos musickits
      ...
      const filteredStickers: musickit[] = musickitsData.filter(
        (musickit: musickit) =>
          ...
          musickit.market_hash_name !== null &&
          musickit.type !== "Pass",
      );

      // Substituindo Axios por Fetch para pegar os dados das crates
      ...
      const stickersWithCrates = filteredStickers.map((musickit) => {
        // Filtrando os crates associados ao musickit
        const associatedCrates = cratesData.filter((crate: Crate) =>
          crate.contains.some((kit: MusicKit) => kit.id === musickit.id),
        );
