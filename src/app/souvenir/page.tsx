...
export default function Souvenir() {
  ...
  const cratesPerPage = 18;

  useEffect(() => {
    const fetchData = async () => {
      try {
        const cratesResponse = await fetch("https://api.cs2data.info/en/crates.json");
        const collectionsResponse = await fetch("https://api.cs2data.info/en/collections.json");

        if (!cratesResponse.ok || !collectionsResponse.ok) {
  ...
  const filteredCrates = crates
    .filter((crate) => crate.type === "Souvenir")
    .reverse();
  ...
  const currentCrates = sortedCrates.slice(indexOfFirstCrate, indexOfLastCrate);
    ...
    <div className="bg-zinc-900">
      <div className="flex justify-center my-10 gap-10 items-center">
        <div className="flex flex-col text-white font-medium px-6 gap-1">
          <span className="text-4xl">All Souvenir Package</span>
        </div>
      </div>
        ...
        {getPageNumbers().map((pageNumber) => (
          <button
            ...
            className={`mx-2 px-4 py-2 rounded ${
              currentPage === pageNumber ? "bg-indigo-600" : "bg-black-300"
            }`}
          >
            {pageNumber}
          </button>
          ...
          {currentCrates.map((crate: Crate) => {
            ...
            return (
              ...
              >
                ...
                >
                  <div className="px-4 my-2 font-medium h-12">
