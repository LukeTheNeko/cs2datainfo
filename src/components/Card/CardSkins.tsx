...
export default function CardSkins(props: CardProps) {
  ...
  const textSizeTitleClass =
    props.skinName.length > 40 ? "text-[12px]" : "text-[15px]";
        ...
        <div className="header">
          ...
          <div className="rarity-info">
            <div
              ...
              style={{
                backgroundColor: props.rarity?.color || "transparent",
              }}
            >
              {/* Exibe o nome da raridade apenas se estiver presente */}
              {props.rarity?.name && <span>{props.rarity.name}</span>}
            </div>
            <div className={`special-option ${rarityClasses[rarityType]}`}>
              {props.specialOption === "StatTrak" && "StatTrak Available"}
              {props.specialOption === "Souvenir" && "Souvenir Available"}
            </div>
          </div>
        </div>
        ...
        <Link
          href={`${props.basePath}/${encodeURIComponent(
            props.collectionName
              .replace(/\|/g, "") // Remove "|"
              .replace(/[()]/g, "")
              .replace(/CS:GO/g, "csgo") // Remove "CS:GO"
