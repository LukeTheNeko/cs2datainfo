                ...
                {isOpen && (
                    <ul className="lg:absolute mt-1 max-h-[450px] overflow-y-auto bg-black-300 z-50 text-white rounded shadow-xl w-full lg:w-auto">
                        {items.map((item, index) => (
                            <div key={index}>
                                {item.miniTitle && (
                                    <div className="text-[12px] text-zinc-400 pl-2 font-semibold my-2">
                                        {item.miniTitle}
                                    </div>
                                )}
                                <a
                                    className="flex items-center hover:bg-black-400 cursor-pointer no-underline pr-4 text-white font-normal text-[14px]"
                                    href={item.link}
                                >
